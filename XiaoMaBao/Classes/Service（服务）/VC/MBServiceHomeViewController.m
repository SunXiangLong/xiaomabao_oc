...
//        MMLog(@"%@",responseObject);
        if (responseObject) {
            if (_page == 1) {
                self.serviceModel = [MBServiceModel yy_modelWithDictionary:responseObject];
                [self setRefresh];
                [self setCollectionViewFlowLayout];
                [self.collectionView reloadData];
            }else{
                 [self.tableView.mj_footer endRefreshing];
                [self.serviceModel.shopsArray addObjectsFromArray:[MBServiceModel yy_modelWithDictionary:responseObject].shopsArray];
            }

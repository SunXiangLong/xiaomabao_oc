...
-(void)setModel:(MBGoodsSpecsModel *)model{
    _model = model;
    [self setUI];
}
-(void)setUI{
    ...
    sepceLbl.text = _model.attr_name;
...
//    NSUInteger count = array.count;
    CGFloat height = 30;
    for (MBGoodsAttrListModel *listModel in _model.goodsAttrList) {
        CGFloat width = [listModel.goods_attr_name sizeWithFont:[UIFont systemFontOfSize:12] withMaxSize:CGSizeMake(MAXFLOAT, 30)].width + 20;
        ...
        btn.tag = [_model.goodsAttrList indexOfObject:listModel];
...
- (void)clickButn:(UIButton *)button{
    ...
    if (_determineGoodsSpecs) {
        self.determineGoodsSpecs(_model);
    }

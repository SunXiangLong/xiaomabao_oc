...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 1010) {
        [self performSegueWithIdentifier:@"MBRegisterPhoneVerifyViewController" sender:nil];
    }
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    
    if ([segue.identifier isEqualToString:@"MBRegisterPhoneVerifyViewController"]) {
        MBRegisterPhoneVerifyViewController *controller = segue.destinationViewController;
        controller.phoneNumb = self.accountField.text;
        controller.phoneVerifyCode = self.phoneCodeMd5;

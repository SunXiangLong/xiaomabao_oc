    ...
    if (self) {
        ...
        for(int i = 0;i<5;i++){
            UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, width*ZOOM, width*ZOOM)];
             img.center = CGPointMake((i+1.5)*width, frame.size.height/2-2);
...
-(void)tap:(UITapGestureRecognizer *)gesture{
    if(self.enable){
        ...
        if (self.delagate &&[self.delagate respondsToSelector:@selector(RatingBar_starNumber:)]) {
            [self.delagate RatingBar_starNumber:_starNumber];
        }

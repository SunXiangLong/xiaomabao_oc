...
+(NSString*)deltaTimeFrom:(NSString*)start end:(NSString*)end{

        NSTimeInterval endTime = [end doubleValue];
//        NSTimeInterval startTime = [start doubleValue];

        NSTimeInterval secondBetweenDates = endTime;
        
        int date = fabs(secondBetweenDates/86400.0) ;
        int hour = fabs((fabs(secondBetweenDates) - date*86400)/3600) ;
        int temp = abs((int)secondBetweenDates%3600) ;
        int minute =abs(temp/60);
        int second =abs(temp%60) ;
        NSString* deltaTimeStr;
        ...
        if (date == 0) {
            ...
            deltaTimeStr = [NSString stringWithFormat:@"剩余%@时",hourStr];
            
        } else {
            deltaTimeStr = [NSString stringWithFormat:@"剩余%d天%@时",date,hourStr];

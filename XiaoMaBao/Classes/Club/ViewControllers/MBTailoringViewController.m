...
@interface MBTailoringViewController ()<UIScrollViewDelegate>
...
@implementation MBTailoringViewController
...
#define kButtonWidth 50
    ...
    self.overlayView.frame = CGRectMake(0, (UISCREEN_HEIGHT-UISCREEN_WIDTH)/2, UISCREEN_WIDTH, UISCREEN_WIDTH);
...
{
    CGRect frame = self.scrollView.frame;//CGRectMake(0, 0, UISCREEN_WIDTH, UISCREEN_WIDTH);
    ...
    if (self.imageView.image) {
        ...
        if (isBaseOnWidth) {
            self.scrollView.contentInset = UIEdgeInsetsMake(CGRectGetMinY(self.overlayView.frame), 0, UISCREEN_HEIGHT-CGRectGetMaxY(self.overlayView.frame), 0);

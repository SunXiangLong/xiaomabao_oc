...
- (void)requestData{
    ...
    [MBNetworking POSTOrigin:@"http://api.xiaomabao.com/feature/detail" parameters:@{@"type":_type,@"id":_ID,@"page":s_Integer((long)_page)} success:^(id responseObject) {
        ...
        if ([responseObject[@"goods_list"] count] > 0) {
            if (_page == 1) {
                _model = [BrandDetailModel yy_modelWithDictionary:responseObject];
            }else{
                [_model.goodsList addObjectsFromArray:[BrandDetailModel yy_modelWithDictionary:responseObject].goodsList];
            }

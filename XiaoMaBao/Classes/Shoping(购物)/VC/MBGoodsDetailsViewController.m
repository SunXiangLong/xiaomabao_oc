...
-(void)goodShare{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.xiaomabao.com/goods-%@?share=APP.html",_model.goods_id]];
...
- (void)removeCollection:(MBGoodsModel *)model{
    ...
    [self show];
    [MBNetworking  POSTOrigin:string(BASE_URL_root, @"/collect/del_goods") parameters:@{@"session":sessiondict,@"goods_id":model.goods_id} success:^(id responseObject) {
//        MMLog(@"%@",responseObject);
        [self dismiss];
